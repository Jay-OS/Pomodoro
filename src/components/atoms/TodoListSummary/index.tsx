...
const TodoListSummaryController = () => {
    ...
    useEffect(
        () => {
            if (todoListState.currentItemIndex !== undefined) {
                const currentItemType = todoListState.withBreaks[todoListState.currentItemIndex].itemType;
    ...
    useEffect(
        () => {
            const totalTimeLeft = todoListState.withBreaks
                ...
                .reduce(
                    (partialSum, item) => partialSum + todoListState.itemDurationsMins[item.itemType],
                    0
                );
    ...
    useEffect(
        () => {
            setCurrentTimerMinutes(Math.ceil(timerState.currentTimer.ellapsedMS / 1000 / 60));
        },
        [timerState.currentTimer.ellapsedMS]
    );

    let totalTodoTimeLeft = (todoCount - completedTodoCount) * todoListState.itemDurationsMins[todoItemTypes.POMODORO];
